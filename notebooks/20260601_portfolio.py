...
command = BuildFeaturesDatasetCommand.model_validate(
    {
        ...
        "clip_quantile": 0.99,
        "seed": 27,
        "artifact_name": "portfolio_etf_macro_features_2026_06.parquet",
...
# It shouldn't matter too much because there is only on row of test for each etf, but in the next update it has to be
#  done with global minmax for the time variable, and then separate regular normalize for the other
# features train/test wise.
...
scenario_returns.to_csv("20260601_gp_posterior_scenarios.csv", index=False)
...
df_sorted.to_csv("20260601_gameday_predictions.csv", index=False)
