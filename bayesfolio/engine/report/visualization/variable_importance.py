...
def xgboost_variable_importance(X, y, xgb_params=None) -> dict:
    ...
    shap_df = pd.DataFrame(shap_main, columns=feature_names, index=X.index)
    ...
    # Create the heatmap
    fig = go.Figure(
        data=go.Heatmap(z=interaction_df.values, x=interaction_df.columns, y=interaction_df.index, colorscale="Viridis")
    )
